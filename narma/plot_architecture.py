...
def draw_architecture(
    ...
    first_x = 2.0
    reservoir_centers = [
        (first_x + index * spacing, center_y) for index in range(reservoirs)
    ]
    latent_centers = [
    ...
    for index, (center, nodes) in enumerate(
        ...
        draw_edges(
            ...
            nodes,
            random_pairs(
                rng, nodes_per_reservoir, nodes_per_reservoir, nodes_per_reservoir * 3
            ),
            FIXED,
            alpha=0.085,
        )
    ...
    input_targets = rng.choice(
        nodes_per_reservoir,
        len(input_sources),
        replace=len(input_sources) > nodes_per_reservoir,
    )
    draw_edges(
        axis,
        input_sources,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("narma/figures/five_reservoir_architecture.png"),
    )
