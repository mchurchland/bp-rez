...
class ExperimentConfig:
    output_dir: str = "narma/results/narma10_300_nodes"
...
def run_one(
    ...
    _save_prediction_plot(
        prediction, target, config.washout, run_dir / "predictions.png"
    )
    return metrics
...
def _save_summary_plot(summary: list[dict[str, Any]], path: Path) -> None:
    ...
    axis.set(
        ylabel="Test NRMSE (mean ± sample SD)", title="NARMA10 multi-seed comparison"
    )
...
def run_experiment(
    ...
    if config.washout >= min(
        config.train_length, config.val_length, config.test_length
    ):
