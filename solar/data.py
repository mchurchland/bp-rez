...
def earth_view_angles(phi_earth: np.ndarray, phi_mars: np.ndarray) -> np.ndarray:
    ...
    distance = np.sqrt(
        ...
        - 2.0 * MARS_ORBIT_RADIUS * EARTH_ORBIT_RADIUS * np.cos(phi_mars - phi_earth)
    )
    sin_theta_mars = (
        EARTH_ORBIT_RADIUS * np.sin(phi_earth) - MARS_ORBIT_RADIUS * np.sin(phi_mars)
    ) / distance
    cos_theta_mars = (
        EARTH_ORBIT_RADIUS * np.cos(phi_earth) - MARS_ORBIT_RADIUS * np.cos(phi_mars)
    ) / distance
...
def _initial_heliocentric_angles(
    ...
    if catalog_size < 2:
        raise ValueError(
            "lifetime_days must contain at least two observation intervals"
        )
    offsets = delta_days * np.arange(catalog_size, dtype=np.float64)
    earth_catalog = (
        EARTH_ANGLE_AT_BIRTH + 2.0 * np.pi * offsets / EARTH_CATALOG_PERIOD_DAYS
    )
