...
from solar.data import SolarDataset, earth_view_angles, make_solar_splits
from solar.models import SolarReservoir, build_solar_model
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--grid-root",
        default="solar/results/solar-grid-15k-108774",
    ...
    parser.add_argument(
        "--output-dir",
        default="solar/results/solar-performance-gap",
    )
...
def _resolve_run_dir(grid_root: Path, explicit_run_dir: str | None) -> Path:
    ...
    recorded = Path(winner["run_dir"])
    if recorded.is_dir():
        return recorded
    relocated = grid_root / "runs" / recorded.name
    if relocated.is_dir():
        return relocated
    raise FileNotFoundError(
        "the selected grid run was not found at either "
        f"{recorded} or the relocation-safe fallback {relocated}"
    )
...
class OfficialSciNet:
    ...
    def __init__(self, checkpoint_prefix: Path) -> None:
        ...
        self.latent_delta = self.graph.get_tensor_by_name("RNN/euler_vars/b_euler0:0")
        self.outputs = [
...
def _error_curves(prediction: np.ndarray, target: np.ndarray) -> dict[str, np.ndarray]:
    squared = (prediction.astype(np.float64) - target.astype(np.float64)) ** 2
...
def _latent_grid(
    ...
    observation[..., 1] = np.unwrap(mars - TWO_PI * branch_offset, discont=5.0).reshape(
        grid_size, grid_size
    )
...
def _latent_diagnostics(
    ...
    diagnostics = {
        ...
        "latent_delta_cosine_similarity": float(
            np.dot(delta, expected_delta) / max(actual_norm * expected_norm, 1e-12)
        ),
    }
...
def _primary_latents(
    ...
    steps = torch.arange(horizon, device=observation.device, dtype=observation.dtype)
...
def _predict_with_readout(
    ...
    return np.concatenate(predictions).reshape(len(dataset), dataset.target.shape[1], 2)
...
def _plot_cumulative_error(
    ...
    for axis, component in zip(axes, ("overall", "sun", "mars"), strict=True):
...
def _plot_examples(
    ...
    for column, index in enumerate(indices):
        for row, (component, label) in enumerate(((0, "Sun"), (1, "Mars"))):
...
def _plot_phase_error(
    ...
    official_error = np.sqrt(np.mean((official_prediction - target) ** 2, axis=(1, 2)))
    phi = np.mod(dataset.heliocentric[:, 0].numpy(), TWO_PI)
    grid_map = _binned_phase_error(phi[:, 0], phi[:, 1], grid_error, bins)
    official_map = _binned_phase_error(phi[:, 0], phi[:, 1], official_error, bins)
...
def _plot_decomposition(
    ...
    values = [100.0 * decompositions[label]["relative_rmse_2pi"] for label in labels]
...
def main() -> None:
    ...
    try:
        ...
        with torch.no_grad():
            grid_surface = (
                model.encode(torch.from_numpy(flattened_observation).to(device))
                .cpu()
                .numpy()
    ...
    expected_delta = np.asarray(grid_latent_metrics["heliocentric_fit_expected_delta"])
    corrected_increment_latents = (
        grid_latents[:, :1, :].astype(np.float64)
        + steps[None, :, None] * expected_delta[None, None, :]
    )
    ...
    decompositions = {
        ...
        "Corrected latent increment": _metrics(corrected_increment_prediction, target),
    ...
    _plot_error_by_week(grid_curves, official_curves, output_dir / "error_by_week.png")
    ...
    _plot_decomposition(decompositions, output_dir / "bottleneck_decomposition.png")
    ...
    report = {
        ...
        "curves": {
            ...
            "official": {key: value.tolist() for key, value in official_curves.items()},
        },
    }
    ...
    print(
        f"Grid winner RMSE/(2pi)={grid_metrics['relative_rmse_2pi']:.4%}",
        flush=True,
    )
    print(
        f"Official SciNet RMSE/(2pi)={official_metrics['relative_rmse_2pi']:.4%}",
        flush=True,
    )
