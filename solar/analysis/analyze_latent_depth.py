...
from solar.data import earth_view_angles, make_solar_splits
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-dir",
        default=("solar/results/solar-10x150-shape-15k-106370/reservoir/seed_0"),
        help="Directory containing checkpoint.pt",
    )
    parser.add_argument(
        "--output-dir",
        default=("solar/results/solar-10x150-shape-15k-106370/latent-depth-analysis"),
    )
...
def _fit_score(
    ...
    augmented = np.column_stack((source.astype(np.float64), np.ones(len(source))))
    ...
    pooled = 1.0 - float(np.sum(squared_residual)) / max(float(np.sum(total)), 1e-12)
...
def _layer_scores(
    ...
    for layer_index in range(latents.shape[1]):
        ...
        geo_forward = _fit_score(geocentric, layer_latent, train_indices, test_indices)
        geo_reverse = _fit_score(layer_latent, geocentric, train_indices, test_indices)
        results.append(
            {
...
def _plot_surfaces(latents: np.ndarray, grid_size: int, path: Path) -> None:
    ...
    for layer_index in range(layer_count):
        for dimension in range(2):
            ...
            axis.set_title(f"After reservoir {layer_index + 1}: latent {dimension + 1}")
...
def main() -> None:
    ...
    report = {
        ...
        "initial_grid_output_mse": float(
            np.mean(
                (grid_prediction.astype(np.float64) - geocentric.astype(np.float64))
                ** 2
            )
        ),
