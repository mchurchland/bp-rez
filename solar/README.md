...
python -m solar.run_experiment \
  ...
  --output-dir solar/results/solar_replication
```
...
python -m solar.run_experiment \
  ...
  --full-dataset-epochs \
  --output-dir solar/results/scinet_literal_replication
```
...
```bash
sbatch --export=ALL,GRID_OUTPUT_DIR=solar/results/solar-grid-15k-JOBID \
  solar/jobs/grid_search.sbatch
```
...
devices can be changed from the CLI; run
`python -m solar.run_experiment --help`.
...
```text
solar/results/solar_replication/
├── config.json
├── metrics.csv
├── metrics.json
├── summary.csv
