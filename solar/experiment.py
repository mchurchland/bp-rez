...
class SolarExperimentConfig:
    ...
    output_dir: str = "solar/results/solar_replication"
    seeds: tuple[int, ...] = (0,)
    data_seed: int = 2026
    ...
    with torch.no_grad():
        for start in range(0, len(dataset), batch_size):
            ...
            if isinstance(model, SolarReservoir):
                ...
                prediction, primary = model.predict_with_latents(observation, horizon)
...
def evaluate_solar_model_with_depth(
    ...
    prediction, primary_latents, initial_latents_by_depth = (
        _batched_prediction_with_depth(model, dataset, horizon, batch_size, device)
    )
    target = dataset.target[:, :horizon].numpy()
    return (
...
def _training_diagnostics(
    ...
    if isinstance(model, SolarReservoir):
        ...
        if len(alphas) > 0:
            diagnostics["intermediate_residual_alphas"] = alphas.detach().cpu().tolist()
        ...
        diagnostics.append(
            {
                ...
                "test_branch_heliocentric_to_latent_r2": (test_helio_to_latent),
                "test_branch_latent_to_heliocentric_r2": (test_latent_to_helio),
...
def _save_latent_depth_plot(diagnostics: list[dict[str, Any]], path: Path) -> None:
    layers = np.asarray([row["latent_index"] for row in diagnostics], dtype=np.int64)
